...
public class Beehiveslot : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void OnPointerClick(PointerEventData eventData){
        ...
        Beehive beehiveinv = beehiveObj.GetComponent<Beehive>();

        int clickedSlot = 0;
        string clickedSlotName = ""+this.gameObject.name;
        string[] splitter = clickedSlotName.Split('_');
        clickedSlot = int.Parse(splitter[1]);
        if (beehiveinv.slots[clickedSlot].isEmpty == false){

            GameObject player = GameObject.Find("Player");
            Inventory inventory = player.GetComponent<Inventory>();

            if(inventory.CheckGetItem(beehiveinv.slots[clickedSlot].itemData) == true){
                inventory.GetItem(beehiveinv.slots[clickedSlot].itemData);
                beehiveinv.slots[clickedSlot].isEmpty = true;
                beehiveinv.slots[clickedSlot].item = null;
                beehiveinv.slots[clickedSlot].itemData = null;
    ...
    public void OnPointerExit(PointerEventData eventData){
        ...
        string[] splitter = clickedSlotName.Split('_');
        clickedSlot = int.Parse(splitter[1]);
        if ((getdragSlot()==clickedSlot)&&(getdragInventory()=="GUI_Beehive")){
            setdragSlot(-1);
