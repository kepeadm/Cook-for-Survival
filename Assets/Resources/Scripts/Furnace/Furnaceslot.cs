...
public class Furnaceslot : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void OnPointerClick(PointerEventData eventData){
        ...
        Furnace furnaceinv = furnaceObj.GetComponent<Furnace>();

        int clickedSlot = 0;
        string clickedSlotName = ""+this.gameObject.name;
        string[] splitter = clickedSlotName.Split('_');
        clickedSlot = int.Parse(splitter[1]);
        if (furnaceinv.slots[clickedSlot].isEmpty == false){

            GameObject player = GameObject.Find("Player");
            Inventory inventory = player.GetComponent<Inventory>();

            if(inventory.CheckGetItem(furnaceinv.slots[clickedSlot].itemData) == true){
                inventory.GetItem(furnaceinv.slots[clickedSlot].itemData, furnaceinv.slots[clickedSlot].potInventory);
                furnaceinv.slots[clickedSlot].isEmpty = true;
                furnaceinv.slots[clickedSlot].item = null;
                furnaceinv.slots[clickedSlot].itemData = null;
    ...
    public void OnPointerExit(PointerEventData eventData){
        ...
        string[] splitter = clickedSlotName.Split('_');
        clickedSlot = int.Parse(splitter[1]);
        if ((getdragSlot()==clickedSlot)&&(getdragInventory()=="GUI_Furnace")){
            setdragSlot(-1);
